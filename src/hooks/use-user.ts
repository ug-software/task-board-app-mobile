...
export default () => {
  ...
  const handleSaveUser = loader.action(async (values: Partial<User>) => {
    try {
      ...
      let isSuccess = await db.insert(UserSchema).values(user);
      ...
      message.schedule({
        phase:
          "Não foi possivel criar usuario nesse momento, tente novamente mais tarde. " +
          error.message,
        severity: "error",
        variant: "container",
      });
  ...
  const handleUpdateUser = loader.action(async (values: Partial<User>) => {
    try {
      ...
      message.schedule({
        phase:
          "Não foi possivel atualizar usuario nesse momento, tente novamente mais tarde. ",
        severity: "error",
        variant: "container",
      });
    }
  ...
  const handleValidationUser = (values: Partial<User>) => {
    ...
    if (
      ...
      errors["description"] =
        "Campo 'Descrição' é de preechimento obrigatório.";
    }
