...
export default () => {
  ...
  const getAllNotifications = loader.action(async () => {
    let notifications = await Notification.findAllPerStatusCreated();
  ...
  const updateNotifications = async () => {
    let today = new Date();
    let notifications = await Notification.findAllPerDate(today);
    ...
    let initialDate = new Date(today.getFullYear(), today.getMonth(), 0);
    ...
    let tasks = await Task.findAllBetweenDates(initialDate, finalDate);
    if (Array.isArray(tasks)) {
      await Promise.all(
        tasks
          .filter(
            (x) =>
              x.status === status.created.name ||
              x.status === status.inprogress.name
          )
          .map(async (task) => {
